...
def password_find(input_list):
    count = 0
    for i in input_list:
        times = i[2].count(i[1])
        if i[0][0] <= times <= i[0][1]:
            count += 1
    return count


def password_find_p2(input_list):
    count = 0
    for i in input_list:
        if i[2][(i[0][0] - 1)] == i[1] or i[2][(i[0][1] - 1)] == i[1]:
            count += 1
        if i[2][(i[0][0] - 1)] == i[1] and i[2][(i[0][1] - 1)] == i[1]:
            count -= 1
    return count


data_clean = sort_data(data)
print(password_find_p2(data_clean))
